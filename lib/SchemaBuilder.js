...
class SchemaBuilder {
  ...
  ensureModelHasTypesProp(model) {
    if (!model.modelClass.GraphqlTypes) {
  ...
  extendWithModelMutations() {
    Object.values(this.models).forEach(model => {
      this.ensureModelHasTypesProp(model)
    });
    const mutations = {};
    Object.values(this.models).forEach(model => {
      const modelMutations = model.modelClass.mutations || {}
      Object.keys(modelMutations).forEach(mutationName => {

        const mutation = modelMutations[mutationName]
        const resolve = mutation.resolve
        mutation.resolve = (...args) => Promise.resolve(resolve(...args)).catch(this.errorHandler)

        mutations[`${model.modelClass.name}_${mutationName}`] = mutation;
      });
    });
    if (Object.values(mutations).length) {
  ...
  extendWithModelSubscriptions() {
    Object.values(this.models).forEach(model => {
      this.ensureModelHasTypesProp(model)
    });
    const subscriptions = {};
    Object.values(this.models).forEach(model => {
      const modelSubscriptions = model.modelClass.subscriptions || {}
      Object.keys(modelSubscriptions).forEach(subscriptionName => {

        const subscription = modelSubscriptions[subscriptionName];
        const subscribe = subscription.subscribe
        subscription.subscribe = (...args) => Promise.resolve(subscribe(...args)).catch(this.errorHandler)

        subscriptions[`${model.modelClass.name}_${subscriptionName}`] = subscription
      });
    });
    if (Object.values(subscriptions).length) {
      this.extendWithSubscriptions(
        new GraphQLObjectType({
          name: "RootSubscriptionType",
          description: "Domain API subscriptions",
  ...
  _rootSingleField(modelData) {
    const resolver = this._resolverForModel(modelData, (query) => {
      query.first();
    })
    return {
      type: this._typeForModel(modelData),
      args: modelData.args,
      resolve: (...args) => Promise.resolve(resolver(...args)).catch(this.errorHandler)
    };
  }

  _rootListField(modelData) {
    const resolver = this._resolverForModel(modelData)
    return {
