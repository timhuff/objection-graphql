...
const { GraphQLObjectType, GraphQLSchema, GraphQLList, GraphQLInputObjectType } = graphqlRoot;
...
const GRAPHQL_META_FIELDS = ["__typename"];
...
class SchemaBuilder {
  ...
  defaultArgNames(defaultArgNameMap) {
    this.defaultArgNameMap = Object.assign(
      {},
      this.defaultArgNameMap,
      defaultArgNameMap
    );
  ...
  extendWithMutations(mutations) {
    if (
      !(mutations instanceof GraphQLObjectType || mutations instanceof Function)
    ) {
      throw new TypeError(
        "mutations should be a function or an object of type GraphQLObjectType"
      );
    }
  ...
  extendWithModelMutations(options) {
    if (!options) options = {};
    if (!options.prefixWithClassName) options.prefixWithClassName = false;
    ...
    const mutations = {};
    Object.values(this.models).forEach(model => {
      const modelMutations = Object.assign({}, model.modelClass.mutations);
      if (options.prefixWithClassName) {
        Object.keys(modelMutations).forEach(mutationName => {
          modelMutations[`${model.modelClass.name}_${mutationName}`] =
            modelMutations[mutationName];
          delete modelMutations[mutationName];
        });
      }
      Object.assign(mutations, modelMutations);
    });
  ...
  build() {
    _.forOwn(this.models, modelData => {
      modelData.fields = jsonSchemaUtils.jsonSchemaToGraphQLFields(
        modelData.modelClass.jsonSchema,
        {
      ...
      query: new GraphQLObjectType({
        ...
        fields: () => {
          ...
          _.forOwn(this.models, modelData => {
            ...
            const listFieldName =
              modelData.opt.listFieldName || `${defaultFieldName}s`;
  ...
  _argsForModel(modelData) {
    const factories = defaultArgFactories(this.defaultArgNameMap, {
      typeCache: this.typeCache
    }).concat(this.argFactories);

    return factories.reduce(
      (args, factory) =>
        Object.assign(args, factory(modelData.fields, modelData.modelClass)),
      {}
    );
  }
  ...
  _typeForModel(modelData) {
    ...
    if (!this.typeCache[typeName]) {
      ...
      this.typeCache[typeName + 'Input'] = new GraphQLInputObjectType({
        name: typeName + 'Input',
        fields: () => Object.assign({}, this._attrFields(modelData)),
      })
    }
  ...
  _relationField(modelData, relation) {
    if (
      relation instanceof objection.HasOneRelation ||
      ...
      relation instanceof objection.HasManyRelation ||
      relation instanceof objection.ManyToManyRelation
    ) {
      return {
        type: new GraphQLList(this._typeForModel(modelData)),
        args: _.omit(modelData.args, OMIT_FROM_RELATION_ARGS)
    ...
    throw new Error(
      `relation type "${relation.constructor.name}" is not supported`
    );
  }
  ...
  _resolverForModel(modelData, extraQuery) {
    return (ctx, ignore1, ignore2, data) => {
      ...
      const argFilter = this._filterForArgs(
        ast,
        modelClass,
        data.variableValues
      );
  ...
  _buildEagerSegment(astNode, modelClass, astRoot) {
    ...
    for (let i = 0, l = astNode.selectionSet.selections.length; i < l; i += 1) {
      ...
      if (relation) {
        expression = this._buildEagerRelationSegment(
          selectionNode,
          relation,
          expression,
          filters,
        ...
        expression = this._buildEagerFragmentSegment(
          selectionNode,
          modelClass,
          expression,
          filters,
  ...
  _buildEagerRelationSegment(
    ...
    const selectFilter = this._filterForSelects(
      selectionNode,
    ...
    const subExpr = this._buildEager(
      selectionNode,
      relation.relatedModelClass,
      astRoot
    );
  ...
  _buildEagerFragmentSegment(
    ...
    const fragmentExpr = this._buildEagerSegment(
      fragmentSelection,
      modelClass,
      astRoot
    );
      ...
      for (let i = 0, l = argObjects.length; i < l; i += 1) {
        ...
        if (
          !(
            ...
            builder.internalOptions().skipUndefined
          )
        ) {
  ...
  _argValue(value, variables) {
    ...
    throw new Error(
      `objection-graphql cannot handle argument value ${JSON.stringify(value)}`
    );
  }
  ...
  _filterForSelects(astNode, modelClass, astRoot) {
    ...
    const relations = modelClass.getRelations();
    const selects = this._collectSelects(
      astNode,
      relations,
      astRoot.fragments,
      []
  ...
  _collectSelects(astNode, relations, fragments, selects) {
    for (let i = 0, l = astNode.selectionSet.selections.length; i < l; i += 1) {
      ...
      if (selectionNode.kind === KIND_FRAGMENT_SPREAD) {
        this._collectSelects(
          fragments[selectionNode.name.value],
          relations,
          fragments,
          selects
        );
        ...
        const relation = relations[selectionNode.name.value];
        const isMetaField =
          GRAPHQL_META_FIELDS.indexOf(selectionNode.name.value) !== -1;
